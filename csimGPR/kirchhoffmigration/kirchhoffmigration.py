...
import csimGPR.kirchhoffmigration.mod_cyMigrate as mod_cyMigrate
...
def PlotImg(data2d,name, ndepth,colormap, nx=1, nz=300, dt=0.002, dx=20, dz=10, dcdp=20, offsets=np.array([0, 500, 1000, 1500, 2000])):
...
# def full_migration(data, nx=200, nz=200, noff=1, dt=0.002, dx=0.05, dz=0.05, dcdp=0.05, V=2e8, offsets=np.array([0, 500, 1000, 1500, 2000])):
#     noff,ntrc,nsmp = data.shape
#     nx = ntrc
#     dcdp=dx
#     offsets = np.arange(0, noff*dx, dx)
#     R = np.zeros((noff,nx, nz))
#     for ioff in range(noff):            # each offset in its own layer
#         print(nx, nz, dt, dx, dz, dcdp, nsmp, ntrc, noff, ioff)
